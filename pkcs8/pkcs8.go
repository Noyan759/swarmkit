...
// ParsePrivateKeyPEMWithPassword parses PEM with password to crypto.signer
...
// ParsePrivateKeyPEM parses PEM to crypto.signer
func ParsePrivateKeyPEM(pemBytes []byte) (crypto.Signer, error) {
	block, _ := pem.Decode(pemBytes)

	return derhelpers.ParsePrivateKeyDER(block.Bytes)
}
...
// DecryptPEMBlock requires PEM Block and password to decrypt and return unencrypted der []byte
...
func encryptPublicKey(pkey, password []byte) ([]byte, error) {
...
// EncryptPEMBlock takes DER-format bytes and password to return an encrypted PEM-block
func EncryptPEMBlock(data, password []byte) (*pem.Block, error) {
	encryptedBytes, err := encryptPublicKey(data, password)
	if err != nil {
		return nil, err
	}
